...
def main():
    ...
    # Load the inputs:
    inputs = LoadInputs()
    construction_history_by_state = inputs.construction_history_by_state
    states_cendivs = inputs.states_cendivs
    cendiv_NEMS = inputs.cendiv_NEMS
    construction_history = inputs.construction_history
    ...
    def print_csv_database_rows(
        ...
        for state in bin_years_sum.keys():
            for year in bin_years_sum[state].keys():
                ...
                print(current_year, state, year, uncovered_floor_space)
...
##        print_single_floor_space_object(_1900)
    ...
    # each aged to the same point in time:
        _1900_to_1903 = create_building_stock(1900, 1903, construction_history)
        age_building_stock_to_year(_1900_to_1903, 1903)
        code_bins = sum_bin_years(_1900_to_1903)
        print_csv_database_rows(1903, code_bins, code_compliance, floor_space_coverage_by_code)
        
        _1900_to_1904 = create_building_stock(1900, 1904, construction_history)
        age_building_stock_to_year(_1900_to_1904, 1904)
        code_bins = sum_bin_years(_1900_to_1904)
        print_csv_database_rows(1904, code_bins, code_compliance, floor_space_coverage_by_code)
