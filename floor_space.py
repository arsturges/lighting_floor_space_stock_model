...
current year, region, age, building type composition, as well
as renovations for all years in its history. In this way, we
can establish objects for each year, and ask those objects
...
class Floor_Space:
    ...
    def __init__(self, year_of_construction, total_initial_square_feet, region):
        ...
        self.remaining_floor_space_by_year = {} # dictionary of dictionaries; dict[year][NEMS_building_type] returns square footage
    ...
    def age_n_years(self, n_years):
        ...
        while end_year > self.current_year:
            ...
            if Floor_Space.verbose: print("Now we'll demolish 1% of each building type from each year:")
            ...
            while year < self.current_year:
                for building_type in [1,2,3,4,5,6,9,10,11,78]:
                    self.remaining_floor_space_by_year[year][building_type] = (
                        self.remaining_floor_space_by_year[year][building_type] - demolished[year][building_type])
                year += 1
            if Floor_Space.verbose: print("That leaves us with:")
            if Floor_Space.verbose: pprint.pprint(self.remaining_floor_space_by_year)

            #renovate some portion of what's left
            if Floor_Space.verbose: print("Of that amount, we'll renovate some, which means")
            if Floor_Space.verbose: print("moving that floor space out of its original year")
            if Floor_Space.verbose: print("and into the current year, whcih is", self.current_year,".")
            ...
            if Floor_Space.verbose: print("Now we'll move that renovated portion",
            ...
            while year < self.current_year:
                for building_type in [1,2,3,4,5,6,9,10,11,78]:
                    self.remaining_floor_space_by_year[year][building_type] = (
                        self.remaining_floor_space_by_year[year][building_type] - stock_renovated_in_current_year[year][building_type])
                year += 1
            ...
            while year < self.current_year:
                for building_type in [1,2,3,4,5,6,9,10,11,78]:
                    if building_type in total_space_renovated_from_all_bins_in_the_current_year:
                        total_space_renovated_from_all_bins_in_the_current_year[building_type] = total_space_renovated_from_all_bins_in_the_current_year[building_type] + stock_renovated_in_current_year[year][building_type]
                    else:
                        total_space_renovated_from_all_bins_in_the_current_year[building_type] = stock_renovated_in_current_year[year][building_type]
                year += 1
            if Floor_Space.verbose: print("The total of all those years is",
    ...
    def renovate(self, floor_space_to_be_renovated):
        ...
        while year < self.current_year:
            ...
            if years_since_last_renovation < 7:
                ...
                rate = 0.1
            for building_type in [1,2,3,4,5,6,9,10,11,78]:
                floor_space_to_be_renovated[year][building_type] = rate * floor_space_to_be_renovated[year][building_type]
            if Floor_Space.verbose: print("The", year, "portion had gone", years_since_last_renovation,
    ...
    def demolish(self, floor_space_to_be_demolished):
        ...
        while year < self.current_year:
            for building_type in [1,2,3,4,5,6,9,10,11,78]:
                floor_space_to_be_demolished[year][building_type] = 0.01 * floor_space_to_be_demolished[year][building_type]
            year += 1
